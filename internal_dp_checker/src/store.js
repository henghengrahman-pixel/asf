...
class Store {
  constructor(dir) {
    ...
    this.balanceBatchesDir = path.join(dir, 'balance-batches');
    fs.mkdirSync(dir, { recursive: true });
    fs.mkdirSync(this.batchesDir, { recursive: true });
    fs.mkdirSync(this.balanceBatchesDir, { recursive: true });
  ...
  _balanceBatchFile(id) {
    const safe = String(id || '').replace(/[^a-zA-Z0-9_-]/g, '');
    return path.join(this.balanceBatchesDir, `${safe}.json`);
  }
  ...
  addBalanceBatch(batch) {
    this._write(this._balanceBatchFile(batch.id), batch);
    return batch;
  }
  getBalanceBatch(id) {
    const file = this._balanceBatchFile(id);
    if (!fs.existsSync(file)) return null;
    return this._read(file, null);
  }
