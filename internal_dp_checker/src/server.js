...
app.use((req, res, next) => {
  res.locals.crmUser = req.session?.user || req.get('X-CRM-User') || 'user';
  res.locals.crmRole = req.session?.crmRole || req.get('X-CRM-Role') || 'leader';
  next();
});
...
app.get('/site/:siteId/balance', auth, siteAuth, (req, res) => res.render('balance', {
  site: req.site, maxIds: MAX_IDS, connected: Boolean(getSiteCookie(req.site)), concurrency: CHECK_CONCURRENCY, retries: CHECK_RETRIES
}));
...
app.get('/balance', auth, firstSiteRedirect('/balance'));
...
async function processBalanceJob(job) {
  const site = store.getSite(job.siteId);
  if (!site) throw new Error('Website sudah tidak tersedia.');
  const cookie = getSiteCookie(site);
  if (!cookie) throw new Error(`Session ${site.name} belum diisi.`);
  ...
  async function worker(workerNo) {
    ...
    while (true) {
      ...
      const userId = job.ids[index], item = job.items[index];
      item.status = 'running'; item.step = 'Mulai'; item.worker = workerNo;
      const result = await checker.checkBalanceOne(userId, (step, meta = {}) => { item.step = step; item.attempt = meta.attempt || item.attempt || 1; item.maxAttempts = meta.maxAttempts || CHECK_RETRIES + 1; });
      job.results[index] = result; item.status = result.status; item.step = 'Selesai'; item.attempt = result.attemptUsed || item.attempt || 1;
      job.completed += 1; job.current = job.completed; job.currentId = userId; job.currentStep = 'Selesai';
    }
  }
  ...
  job.summary = {
    totalIds: job.results.length,
    valid: job.results.filter(x => x.status === 'valid').length,
    invalid: job.results.filter(x => x.status === 'invalid').length,
    error: job.results.filter(x => x.status === 'error').length,
    totalBalance: job.results.filter(x => x.status === 'valid').reduce((sum, x) => sum + Number(x.balance || 0), 0)
  };
  job.status = 'done'; job.finishedAt = nowWib();
  store.addBalanceBatch({ ...job });
  const cleanup = setTimeout(() => jobs.delete(job.id), 30 * 60 * 1000); if (typeof cleanup.unref === 'function') cleanup.unref();
}
...
async function runQueue() {
  ...
  working = true;
  while (queue.length) { const job = queue.shift(); try { if (job.type === 'balance') await processBalanceJob(job); else await processJob(job); } catch (e) { job.status = 'failed'; job.error = e.message; job.finishedAt = nowWib(); } }
  working = false;
}
...
app.post('/site/:siteId/api/balance-jobs', auth, siteAuth, (req, res) => {
  const raw = Array.isArray(req.body.ids) ? req.body.ids.join('\n') : String(req.body.ids || '');
  const ids = [...new Set(raw.split(/[\n,;\s]+/).map(x => x.trim()).filter(Boolean))];
  if (!ids.length) return res.status(400).json({ ok: false, error: 'Masukkan minimal 1 UserId.' });
  if (ids.length > MAX_IDS) return res.status(400).json({ ok: false, error: `Maksimal ${MAX_IDS} UserId per batch.` });
  if (!getSiteCookie(req.site)) return res.status(400).json({ ok: false, error: `Session ${req.site.name} belum diisi. Buka menu Session Agent.` });
  const id = crypto.randomUUID();
  const job = { id, type: 'balance', siteId: req.site.id, siteName: req.site.name, ids, status: 'queued', createdAt: nowWib(), startedAt: null, finishedAt: null, current: 0, completed: 0, currentId: '', currentStep: '', results: new Array(ids.length), summary: null, error: null, items: ids.map(userId => ({ userId, status: 'queued', step: 'Menunggu', attempt: 0, maxAttempts: CHECK_RETRIES + 1, worker: null })) };
  jobs.set(id, job); queue.push(job); runQueue();
  res.json({ ok: true, jobId: id });
});
app.get('/api/balance-jobs/:id', auth, (req, res) => {
  const job = jobs.get(req.params.id);
  if (!job) {
    const old = store.getBalanceBatch(req.params.id); if (!old) return res.status(404).json({ ok: false, error: 'Job saldo tidak ditemukan.' });
  ...
  if (job.type !== 'balance') return res.status(404).json({ ok: false, error: 'Job saldo tidak ditemukan.' });
  res.json({ ok: true, job: { ...job, results: job.results.filter(Boolean) } });
});
app.get('/balance-result/:id', auth, (req, res) => {
  const live = jobs.get(req.params.id);
  const saved = store.getBalanceBatch(req.params.id);
  const data = saved || (live && live.type === 'balance' && live.status === 'done' ? { ...live } : null);
  if (!data) return res.status(404).send('Hasil cek saldo tidak ditemukan atau masih diproses.');
  const site = store.getSite(data.siteId || 'omtogel');
  res.render('balance-result', { batch: data, site: site || { id: data.siteId || 'omtogel', name: data.siteName || 'WEBSITE' } });
});
app.get('/balance-export/:id.csv', auth, (req, res) => {
  const batch = store.getBalanceBatch(req.params.id) || (jobs.get(req.params.id)?.type === 'balance' && jobs.get(req.params.id)?.status === 'done' ? jobs.get(req.params.id) : null);
  if (!batch) return res.status(404).send('Hasil cek saldo tidak ditemukan.');
  const esc = x => `"${String(x ?? '').replace(/"/g, '""')}"`;
  const lines = [['No','Website','UserId','Status','Keterangan','Saldo'].map(esc).join(',')];
  (batch.results || []).forEach((r, i) => lines.push([i+1,batch.siteName||batch.siteId||'',r.userId,r.status,r.keterangan,r.balance||0].map(esc).join(',')));
  res.setHeader('Content-Type', 'text/csv; charset=utf-8'); res.setHeader('Content-Disposition', `attachment; filename="saldo-member-${batch.id.slice(0,8)}.csv"`); res.send('\uFEFF' + lines.join('\r\n'));
});
...
app.listen(PORT, BIND_HOST, () => {
  console.log(`DP Checker Multi-Website internal running on ${BIND_HOST}:${PORT}`);
