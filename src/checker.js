...
const { parseCoin, classifyDeposits } = require('./logic');
...
async function createChecker({ baseUrl, cookieHeader, timeoutMs = 15000 }) {
  ...
  async function checkOne(userId, onStep = () => {}) {
    ...
    try {
      ...
      result.transaksi = classified.valid;
      result.bonus50 = classified.bonus50;
      result.diabaikan = classified.ignored;
      result.jumlahDp = classified.valid.length;
      result.totalDp = classified.valid.reduce((s, x) => s + x.coin, 0);
      result.jumlahBonus50 = classified.bonus50.length;
      result.totalBonus50 = classified.bonus50.reduce((s, x) => s + x.coin, 0);
      result.status = classified.valid.length ? 'has_dp' : 'no_dp';
      result.keterangan = classified.valid.length ? 'Ada DP hari ini' : 'Tidak ada DP hari ini';
      return result;
    } catch (e) {
      result.status = 'error';
