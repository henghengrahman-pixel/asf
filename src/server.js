...
app.get('/export/:id.csv', auth, (req, res) => {
  ...
  const lines = [['No','UserId','Status','Keterangan','Jumlah DP Valid','Total DP Valid','Jumlah Bonus 50%','Total Bonus 50%'].map(esc).join(',')];
  batch.results.forEach((r, i) => lines.push([i+1,r.userId,r.status,r.keterangan,r.jumlahDp,r.totalDp,r.jumlahBonus50 || 0,r.totalBonus50 || 0].map(esc).join(',')));
